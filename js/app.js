...
// ============================================================
import { brandHTML, ICONS, CATEGORY_ICONS, waLink, escapeHtml, toast, CAMARA_WA } from "./ui.js";
...
async function init(){
  ...
  PROV = PROV.map(p=>Object.assign({fotos:[],descripcion:"",badge:"",promo:"",destacado:false,categoria:"Otros",color:"#1877E6"},p));
...
function go(nav){
  closeDrawer();
  switch(nav){
    case "inicio": showView("inicio"); setBottom("inicio"); break;
...
function openList(title, sub, list){
  ...
  if(!list.length){
    grid.innerHTML = `<div class="empty">${ICONS.search}<div>No hay proveedores para mostrar.</div></div>`;
  } else {
    grid.innerHTML = list.slice().sort((a,b)=>(a.nombre||"").localeCompare(b.nombre||"","es")).map(p=>`
...
function wireSearch(){
  ...
  inp.addEventListener("input", ()=>{
    ...
    t = setTimeout(()=>{
      ...
      openList(`Resultados`, `${res.length} para “${inp.value.trim()}”`, res);
      setBottom("");
    }, 180);
  });
}
